...
\begin{lemma}
 For each sequence $(X^t)$, if $\xinf$ exists then it is a fixpoint.
\end{lemma}

\begin{proof}
  By definition, for all position $X_{i,j}$ that is not a constant:
   \[\begin{split}
...
\begin{proof}
  Let $x^{\infty} = \lim_{t \rightarrow \infty} \delta^{\dag^t}(X)_{i,j})$. The fixed point $\xinf$ (limit of $(X^t)$ verifies the following equations:
  ...
  
  These equations are the same than in property \ref{twocst}. Thus, for all $i,j$, $\xinf_{i,j} = x^{\infty}$.
  
  However, according to lemma \ref{conservative} the sum of all values do not change at any step of computation. Thus, $x^{\infty}$ must be equal to the average value of all the initial cells in $X^0$.  
\end{proof}
