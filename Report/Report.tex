...
\]  
  ...
  According to question 3, the complexity in general case is $O(N^2 \cdot t)$. The algorithm using the linearity of $\ddelta$ is more interesting when the number of steps $t$ increases.
\end{proof}
...
\begin{proof}
 We recall that $n = \frac{N}{p}$.
  ...
  At each step of the \texttt{for} loop, each processor performs $O(n^2 \cdot N^2)$ operations according to equation \ref{avequnit}. The final computation of $\ddeltat(X)$ using $\delta^{\dag^{t/2}}(E^{0,0})$ needs $O(N^2 \cdot N^4)$ operations. Thus $T_t(t,N) = \log(t) \cdot p^2 \cdot n^2 \cdot N^2 + N^6) = O(\log(t) \cdot N^4 + N^6)$.
\end{proof}

\begin{prop}
 The space complexity is $T_s(t,N) = O(N^2)$.
\end{prop}

\begin{proof}
 Each processor stores 3 arrays of size $n^2$ and need $O(n^2)$ extra space during broadcast operations. Moreover, this space can be reused at each new step. Thus:
  \[
    \begin{split}
        T_s(t,N) & = O( p^2 \cdot n^2) \\
                 & = O( N^2 )
    \end{split}
 \] 
\end{proof}
...
\begin{prop}
 The communication time complexity is $T_c(t,N) = O( c \cdot p^2 \cdot \log(t) \cdot N^2)$.
\end{prop}

\begin{proof}
 At each step of the \texttt{for} loop, each processor performs $O(p^2)$ broadcasts of a matrix of size $n^2$. Thus:
  \[
    \begin{split}
        T_c(t,N) & = O( c \cdot p^2 \cdot p^2 \cdot \log(t) \cdot n^2) \\
                 & = O( c \cdot p^2 \cdot \log(t) \cdot N^2)
    \end{split}
 \] 
\end{proof}
            ...
            \For{ r=0 \textbf{to} n-1}{
                \For{ k=0 \textbf{to} N-1}{
                    \For{ l=0 \textbf{to} N-1}{
                        $(p_{r_1},\_,\_) \leftarrow$ \texttt{LocalizeCell((k,l))} \\
                        $(p_{r_2},\_,\_) \leftarrow$ \texttt{LocalizeCell(LocalizeProc(p,q,r)-(k,l))} \\
                        \texttt{Send($p_s$,$(q_s,r_s)$,A[l][m])} \\
                        \texttt{Send($p_s$,$(q_s,r_s)$,A[l][m])} \\
                        \For{ all $(s,t) \in [0,n-1]^2$}{
                            $(p_{r_1},\_,\_) \leftarrow$ \texttt{LocalizeCell((k,l))} \\
                            $(p_{r_2},\_,\_) \leftarrow$ \texttt{LocalizeCell(LocalizeProc(p,q,r)-(k,l))} \\
                            \texttt{Send($p_s$,$(q_s,r_s)$,A[l][m])} \\
                            \texttt{Send($p_s$,$(q_s,r_s)$,A[l][m])} \\                          
                        }
                        \texttt{Receive($p_{r_1}$,$(q,r)$,temp1)} \\
                        \texttt{Receive($p_{r_1}$,$(q,r)$,temp2)} \\
                        \texttt{B[q][r] $\leftarrow$ B[q][r] + temp1 $\cdot$ temp2
                        }
                    }
                }
            }
...
%We use the extra functions defined into question 6. 
