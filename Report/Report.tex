...
\begin{proof}
  ...
  These equations are the same than the ones in property \ref{twocst}. Thus, for all $i,j$, $\xinf_{i,j} = x^{\infty}$.
  
  However, according to lemma \ref{conservative} the sum of all values does not change at any step of the computation. Thus, $x^{\infty}$ must be equal to the average value of all the initial cells in $X^0$.  
\end{proof}
