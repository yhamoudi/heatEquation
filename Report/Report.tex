...
We consider functions \texttt{Send\_X} (resp. \texttt{Receive\_X}) for \texttt{X = Top, Bottom, Left, Right} which sends (resp. receives) to (resp. from) the corresponding processor. 
...
We also consider functions to send an entire row (resp. column) as one single message, to decrease the overall latency. For instance, \texttt{Send\_Bottom\_Row(n,PREV)} will send to the bottom processor the $\text{n}^{th}$ row, whereas \texttt{Send\_Top\_Row(0,PREV)} will receive from the top processor a row, which will be stored as the $\text{0}^{th}$ row.
...
\begin{algorithm}[H]
\SetKwFunction{sndtop}{Send\_Top}
\SetKwFunction{sndbot}{Send\_Bottom}
\SetKwFunction{sndleft}{Send\_Left}
\SetKwFunction{sndright}{Send\_Right}
\SetKwFunction{rcvtop}{Receive\_Top}
\SetKwFunction{rcvbot}{Receive\_Bottom}
\SetKwFunction{rcvleft}{Receive\_Left}
\SetKwFunction{rcvright}{Receive\_Right}
\SetKwFunction{sndtopr}{Send\_Top\_Row}
\SetKwFunction{sndbotr}{Send\_Bottom\_Row}
\SetKwFunction{sndleftc}{Send\_Left\_Column}
\SetKwFunction{sndrightc}{Send\_Right\_Column}
\SetKwFunction{rcvtopr}{Receive\_Top\_Row}
\SetKwFunction{rcvbotr}{Receive\_Bottom\_Row}
