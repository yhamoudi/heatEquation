...
\begin{itemize}
 \item \texttt{LocalizeCell(i,j)}: returns the number $p$ of the processor storing $Z^t_{i,j}$ (resp. $X^t_{i,j}$).
 ...
 \item \texttt{Send(p,msg)}: send message \texttt{msg} at processor \texttt{p}.
 \item \texttt{Receive(p,dest)}: receive message sent by processor \texttt{p} ant store it in \texttt{dest}.
\end{itemize}
...
\begin{algorithm}[H]
    \DontPrintSemicolon
    \LinesNotNumbered
    var L: list of non null cells of $X^0$; \\
    ...
    \For{ all $(i,j,X^0_{i,j})$ taken in L}{
        \For{ l=0 \textbf{to} n-1}{
            \For{ m=0 \textbf{to} n-1}{
                $p_s \leftarrow$ \texttt{LocalizeCell(LocalizeProc(p,l,m)+(i,j))} ; \\
                $p_r \leftarrow$ \texttt{LocalizeCell(LocalizeProc(p,l,m)-(i,j))} ; \\
                \texttt{Send($p_s$,A[l][m])} || \texttt{Receive($p_r$,temp)}; \\
                B[l][m] $\leftarrow$ B[l][m] + $X^0_{i,j} \cdot$ temp
            }
        }
    }
...
Initially, all the cells of $B$ must be null. If $p_s = p$ we do not perform the \texttt{Send}. If $p_r = p$, we replace the \texttt{Receive} by $temp \leftarrow A[q_r][r_r]$.
