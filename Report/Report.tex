...
The following algorithm computes $X^t$. \texttt{LocalizeCell(i,j)} returns the number of the processor storing $Z^t_{i,j}$. \texttt{LocalizeProc(p)} returns the position $(i,j)$ of the top left cell of $Z^t$ (or identically, of $X^t$) stored in processor $p$. Thus the submatrix of $Z^t$ stored in processor $p$ is:
\[
   \begin{array}{ | c | c | c |} \hline
    Z^t_{\texttt{LocalizeProc(p)}} & \cdots & Z^t_{\texttt{LocalizeProc(p) + (0,n)}}\\ \hline
    \vdots & \vdots & \vdots \\ \hline
    Z^t_{\texttt{LocalizeProc(p) + (n,0)}} & \cdots & Z^t_{\texttt{LocalizeProc(p) + (n,n)}}\\ \hline
  \end{array}
\]
